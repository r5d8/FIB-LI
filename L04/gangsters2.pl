...
satVariable( works(G,H) ):- gangster(G), hour(H).  %  means:  "gangster G works at hour H"
...
joinDoesWorks() :- gangster(G), hour(H), findall(does(G, T, H), task(T), L), expressOr(works(G, H), L), fail.
joinDoesWorks().
...
costOfThisSolution(M,Cost):- 
    findall(H_consec, (member(works(G, H), M), hores_consec(G, H, M, H_consec)), L),
    max_list(L, Cost).
