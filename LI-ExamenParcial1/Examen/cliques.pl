...
main:- vertices(Vs), subconjunto( Vs, S), minCliqueSize(MSize), 
        length(S, L), L >= MSize, isClique(S), write(S), nl, fail.
main:- halt.
...
isClique([]).
isClique([X|S]) :- 
    vertex(X), findall(V, (vertex(V), member(V, S), edge(X, V)), L),
    length(L, Size), length(S, Size), isClique(S).
