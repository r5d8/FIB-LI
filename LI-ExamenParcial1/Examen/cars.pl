...
relateCarPosAndPosOpt :- car(C), position(P), carOption(C, Opt), writeClause([-carPos(C, P), posOpt(P, Opt)]), fail.
relateCarPosAndPosOpt.

optionConstraints:-
    option(Opt,Max,WindowSize),
    position(Ini),
    windowOfSize(WindowSize, Ini, WindowEnd),
    %findall(carPos(C, P), (between(Ini, WindowEnd, P), carOption(C, Opt)), L), %Solució sense usar posOpt, que genera 1.805.812 clàusules
    findall(posOpt(P, Opt), between(Ini, WindowEnd, P), L), %Solució usant posOpt, que genera 3.926 clàusules
    atMost(Max, L), fail.
optionConstraints.

oneCarPerPos :- car(C), findall(carPos(C, P), position(P), L), exactly(1, L), fail.
oneCarPerPos.

onePosPerCar :- position(P), findall(carPos(C, P), car(C), L), exactly(1, L), fail.
onePosPerCar.
